...
class Community {
 public:
  ...
  vector<double> coefficient; //input node coefficient
  vector<double> commCoeff; //input comm coefficient
  ...
  Community (char *filename, char *filename_w, char * filename_coe, int type, int nb_pass, double min_modularity);
  // copy graph
  ...
  inline static bool great_second(const Pair & m1, const Pair & m2);
  ...
  inline int getCommSize( int comm );
  ...
  bool one_level();
  bool one_level( int level, char * filename );
};
